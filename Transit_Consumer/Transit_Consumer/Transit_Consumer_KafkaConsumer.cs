...
{
    public class Transit_Consumer_KafkaConsumer
    {
        /// <summary>
        ///     In this example
        ...
        {
            var sBrokerList = Transit_Consumer_Globals.sKafkaBrokerList;
            var sKafkaTopics = Transit_Consumer_Globals.sKafkaTopic;

            Transit_Consumer_Globals.LogFile.Log("Start Kafka Consumer at: " + sBrokerList + ", listen on topic(s): " + sKafkaTopics);
        ...
        {
            int iCount = 0;
           // Transit_Consumer_DataStructures.Transit_Consumer_LinkData _LinkData = new Transit_Consumer_DataStructures.Transit_Consumer_LinkData();
            string sVal = "";
            ...
            var config = new ConsumerConfig
            {
                ...
                GroupId = Transit_Consumer_Globals.sKafkaConsumerGroup,
                ...
                try
                {
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            if (!bPrintedOffset)
                            {
                                Transit_Consumer_Globals.LogFile.Log("Launched consumer for topic " + consumeResult.Topic + ", beginning at offset: " + consumeResult.Offset + ".");
                                bPrintedOffset = true;
                            }

                            if (consumeResult.IsPartitionEOF)
                            {
                                Transit_Consumer_Globals.LogFile.Log("Reached end of topic " + consumeResult.Topic + ", partition " + 
                               ...
                               // consumer.StoreOffset(consumeResult);
                                ...
                                if (lstData.Count > 0)
                                {
                                    Transit_Consumer_HandleResponse.HandleResponse(lstData);
                           ...
                           // {
                            ...
                            try
                            {
                                ...
                                {
                                    iCount++;
                                    Transit_Consumer_Globals.LogFile.Log("Begin receiving data on topic " + Transit_Consumer_Globals.sKafkaTopic + 
