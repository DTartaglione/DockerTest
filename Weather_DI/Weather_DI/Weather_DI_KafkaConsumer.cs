...
{
    public class Weather_DI_KafkaConsumer
    {
        /// <summary>
        ///     In this example
        ...
        {
            var sBrokerList = Weather_DI_Globals.sKafkaBrokerList;
            var sKafkaTopics = Weather_DI_Globals.sKafkaTopic;

            Weather_DI_Globals.LogFile.Log("Start Kafka Consumer at: " + sBrokerList + ", listen on topic(s): " + sKafkaTopics);
        ...
        public static void Run_Consume(string brokerList, List<string> topics)
        {
            ...
            var config = new ConsumerConfig
            {
                ...
                GroupId = Weather_DI_Globals.sKafkaConsumerGroup,
                ...
                SecurityProtocol = (SecurityProtocol)Weather_DI_Globals.iSecurityProtocol, //kafka needs 0, event hubs 3
                SaslMechanism = (SaslMechanism)Weather_DI_Globals.iSaslMechanism, //Kafka needs 1, event hubs 1
                SaslUsername = Weather_DI_Globals.sSaslUsername,
                SaslPassword = Weather_DI_Globals.sSaslPassword
            };
                ...
                try
                {
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            if (Weather_DI_Globals.iSecurityProtocol == 0)
                            {
                                ...
                                if (!bPrintedOffset)
                                {
                                    Weather_DI_Globals.LogFile.Log("Launched consumer for topic " + consumeResult.Topic + ", beginning at offset: " + consumeResult.Offset + ".");
                                    bPrintedOffset = true;
                                }

                                if (consumeResult.IsPartitionEOF)
                                {
                                    Weather_DI_Globals.LogFile.Log("Reached end of topic " + consumeResult.Topic + ", partition " +
                                           consumeResult.Partition + ", offset " + consumeResult.Offset + ".");

                                    if (lstData.Count > 0)
                                    {
                                        Weather_DI_HandleResponse.HandleWeatherResponse(lstData);
                                ...
                                try
                                {
                                    ...
                                    {
                                        iCount++;
                                        Weather_DI_Globals.LogFile.Log("Begin receiving data on topic " + Weather_DI_Globals.sKafkaTopic +
                            ...
                            else if (Weather_DI_Globals.iSecurityProtocol == 3)
                            {
                                //Event Hubs consume result. Need to use timeout vs. cancellation token. Cancellation Token does not work with Event Hubs
                                consumeResult = consumer.Consume(Weather_DI_Globals.iKafkaConsumerConsumeTimeoutInMillieconds);
