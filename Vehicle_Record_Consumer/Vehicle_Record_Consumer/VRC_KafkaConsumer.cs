...
{
    public class Vehicle_Record_Consumer_KafkaConsumer
    {
        /// <summary>
        ///     In this example
        ...
        {
            var sBrokerList = Vehicle_Record_Consumer_Globals.sKafkaBrokerList;
            var sKafkaTopics = Vehicle_Record_Consumer_Globals.sKafkaTopic;

            Vehicle_Record_Consumer_Globals.LogFile.Log("Start Kafka Consumer at: " + sBrokerList + ", listen on topic(s): " + sKafkaTopics);
        ...
        public static void Run_Consume(string brokerList, List<string> topics)
        {
            ...
            var config = new ConsumerConfig
            {
                ...
                GroupId = Vehicle_Record_Consumer_Globals.sKafkaConsumerGroup,
                ...
                SecurityProtocol = (SecurityProtocol)Vehicle_Record_Consumer_Globals.iSecurityProtocol, //kafka needs 0, event hubs 3
                SaslMechanism = (SaslMechanism)Vehicle_Record_Consumer_Globals.iSaslMechanism, //Kafka needs 1, event hubs 1
                SaslUsername = Vehicle_Record_Consumer_Globals.sSaslUsername,
                SaslPassword = Vehicle_Record_Consumer_Globals.sSaslPassword
            };
                ...
                try
                {
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            if (Vehicle_Record_Consumer_Globals.iSecurityProtocol == 0)
                            {
                                ...
                                if (!bPrintedOffset)
                                {
                                    Vehicle_Record_Consumer_Globals.LogFile.Log("Launched consumer for topic " + consumeResult.Topic + ", beginning at offset: " + consumeResult.Offset + ".");
                                    bPrintedOffset = true;
                                }

                                if (consumeResult.IsPartitionEOF)
                                {
                                    Vehicle_Record_Consumer_Globals.LogFile.Log("Reached end of topic " + consumeResult.Topic + ", partition " +
                                           consumeResult.Partition + ", offset " + consumeResult.Offset + ".");

                                    if (lstData.Count > 0)
                                    {
                                        Vehicle_Record_Consumer_HandleResponse.HandleResponse(lstData);
                                ...
                                try
                                {
                                    ...
                                    {
                                        iCount++;
                                        Vehicle_Record_Consumer_Globals.LogFile.Log("Begin receiving data on topic " + Vehicle_Record_Consumer_Globals.sKafkaTopic +
                            ...
                            else if (Vehicle_Record_Consumer_Globals.iSecurityProtocol == 3)
                            {
                                //Event Hubs consume result. Need to use timeout vs. cancellation token. Cancellation Token does not work with Event Hubs
                                consumeResult = consumer.Consume(Vehicle_Record_Consumer_Globals.iKafkaConsumerConsumeTimeoutInMillieconds);
