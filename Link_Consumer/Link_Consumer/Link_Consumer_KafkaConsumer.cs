...
{
    public class Link_Consumer_KafkaConsumer
    {
        /// <summary>
        ///     In this example
        ...
        {
            var sBrokerList = Link_Consumer_Globals.sKafkaBrokerList;
            var sKafkaTopics = Link_Consumer_Globals.sKafkaLinkTopic;

            Link_Consumer_Globals.LogFile.Log("Start Kafka Consumer at: " + sBrokerList + ", listen on topic(s): " + sKafkaTopics);
        ...
        public static void Run_Consume(string brokerList, List<string> topics)
        {
            ...
            var config = new ConsumerConfig
            {
                ...
                GroupId = Link_Consumer_Globals.sKafkaConsumerGroup,
                ...
                SecurityProtocol = (SecurityProtocol)Link_Consumer_Globals.iSecurityProtocol, //kafka needs 0, event hubs 3
                SaslMechanism = (SaslMechanism)Link_Consumer_Globals.iSaslMechanism, //Kafka needs 1, event hubs 1
                SaslUsername = Link_Consumer_Globals.sSaslUsername,
                SaslPassword = Link_Consumer_Globals.sSaslPassword
            };
                ...
                try
                {
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            if (Link_Consumer_Globals.iSecurityProtocol == 0)
                            {
                                ...
                                if (!bPrintedOffset)
                                {
                                    Link_Consumer_Globals.LogFile.Log("Launched consumer for topic " + consumeResult.Topic + ", beginning at offset: " + consumeResult.Offset + ".");
                                    bPrintedOffset = true;
                                }

                                if (consumeResult.IsPartitionEOF)
                                {
                                    Link_Consumer_Globals.LogFile.Log("Reached end of topic " + consumeResult.Topic + ", partition " +
                                           consumeResult.Partition + ", offset " + consumeResult.Offset + ".");

                                    if (lstData.Count > 0)
                                    {
                                        Link_Consumer_HandleLinkStatusResponse.HandleLinkStatusResponse(lstData);
                                ...
                                try
                                {
                                    ...
                                    {
                                        iCount++;
                                        Link_Consumer_Globals.LogFile.Log("Begin receiving data on topic " + Link_Consumer_Globals.sKafkaLinkTopic +
                            ...
                            else if (Link_Consumer_Globals.iSecurityProtocol == 3)
                            {
                                //Event Hubs consume result. Need to use timeout vs. cancellation token. Cancellation Token does not work with Event Hubs
                                consumeResult = consumer.Consume(Link_Consumer_Globals.iKafkaConsumerConsumeTimeoutInMillieconds);
