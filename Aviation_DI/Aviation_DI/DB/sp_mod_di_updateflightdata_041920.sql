--001 - Drop existing function because we are adding a new column
drop function if exists db_objects.sp_mod_di_updateflightdata;

--002 - New replaced function with new flight_type_id column added
...
_departure_gate character varying, _scheduled_arrival_datetime character varying, _estimated_arrival_datetime character varying, _actual_arrival_datetime character varying, 
_arrival_gate character varying, _latitude double precision, _longitude double precision, _direction double precision, _departure_delay integer, _arrival_delay integer, 
_num_seats integer, _last_update timestamp without time zone, _flight_type_id int4)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
...
if _scheduled_departure_datetime = '' then
	   ...
	   BEGIN
		    ...
		    direction, departure_delay, arrival_delay, number_seats, geom, last_update, flight_type_id)
		    VALUES
		    ...
		    _direction, _departure_delay, _arrival_delay, _num_seats, _geom, _last_update, _flight_type_id);
	       END;
	   ELSE 
	       BEGIN
