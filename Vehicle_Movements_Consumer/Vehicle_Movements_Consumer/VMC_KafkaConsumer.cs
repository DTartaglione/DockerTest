...
{
    public class Vehicle_Movements_Consumer_KafkaConsumer
    {
        /// <summary>
        ///     In this example
        ...
        {
            var sBrokerList = Vehicle_Movements_Consumer_Globals.sKafkaBrokerList;
            var sKafkaTopics = Vehicle_Movements_Consumer_Globals.sKafkaTopic;

            Vehicle_Movements_Consumer_Globals.LogFile.Log("Start Kafka Consumer at: " + sBrokerList + ", listen on topic(s): " + sKafkaTopics);
        ...
        {
            int iCount = 0;
           // Vehicle_Movements_Consumer_DataStructures.Vehicle_Movements_Consumer_LinkData _LinkData = new Vehicle_Movements_Consumer_DataStructures.Vehicle_Movements_Consumer_LinkData();
            string sVal = "";
            ...
            var config = new ConsumerConfig
            {
                ...
                GroupId = Vehicle_Movements_Consumer_Globals.sKafkaConsumerGroup,
                ...
                try
                {
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            if (!bPrintedOffset)
                            {
                                Vehicle_Movements_Consumer_Globals.LogFile.Log("Launched consumer for topic " + consumeResult.Topic + ", beginning at offset: " + consumeResult.Offset + ".");
                                bPrintedOffset = true;
                            }

                            if (consumeResult.IsPartitionEOF)
                            {
                                Vehicle_Movements_Consumer_Globals.LogFile.Log("Reached end of topic " + consumeResult.Topic + ", partition " + 
                               ...
                               // consumer.StoreOffset(consumeResult);
                                ...
                                if (lstData.Count > 0)
                                {
                                    Vehicle_Movements_Consumer_HandleResponse.HandleResponse(lstData);
                           ...
                           // {
                            ...
                            try
                            {
                                ...
                                {
                                    iCount++;
                                    Vehicle_Movements_Consumer_Globals.LogFile.Log("Begin receiving data on topic " + Vehicle_Movements_Consumer_Globals.sKafkaTopic + 
