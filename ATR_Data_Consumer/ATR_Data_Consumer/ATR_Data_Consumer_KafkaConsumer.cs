...
{
    public class ATR_Data_Consumer_KafkaConsumer
    {
        /// <summary>
        ///     In this example
        ...
        {
            var sBrokerList = ATR_Data_Consumer_Globals.sKafkaBrokerList;
            var sKafkaTopics = ATR_Data_Consumer_Globals.sKafkaTopic;

            ATR_Data_Consumer_Globals.LogFile.Log("Start Kafka Consumer at: " + sBrokerList + ", listen on topic(s): " + sKafkaTopics);
        ...
        public static void Run_Consume(string brokerList, List<string> topics)
        {
            ...
            var config = new ConsumerConfig
            {
                ...
                GroupId = ATR_Data_Consumer_Globals.sKafkaConsumerGroup,
                ...
                SecurityProtocol = (SecurityProtocol)ATR_Data_Consumer_Globals.iSecurityProtocol, //kafka needs 0, event hubs 3
                SaslMechanism = (SaslMechanism)ATR_Data_Consumer_Globals.iSaslMechanism, //Kafka needs 1, event hubs 1
                SaslUsername = ATR_Data_Consumer_Globals.sSaslUsername,
                SaslPassword = ATR_Data_Consumer_Globals.sSaslPassword
            };
                ...
                try
                {
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            if (ATR_Data_Consumer_Globals.iSecurityProtocol == 0)
                            {
                                ...
                                if (!bPrintedOffset)
                                {
                                    ATR_Data_Consumer_Globals.LogFile.Log("Launched consumer for topic " + consumeResult.Topic + ", beginning at offset: " + consumeResult.Offset + ".");
                                    bPrintedOffset = true;
                                }

                                if (consumeResult.IsPartitionEOF)
                                {
                                    ATR_Data_Consumer_Globals.LogFile.Log("Reached end of topic " + consumeResult.Topic + ", partition " +
                                           consumeResult.Partition + ", offset " + consumeResult.Offset + ".");

                                    if (lstData.Count > 0)
                                    {
                                        ATR_Data_Consumer_HandleResponse.HandleResponse(lstData);
                                ...
                                try
                                {
                                    ...
                                    {
                                        iCount++;
                                        ATR_Data_Consumer_Globals.LogFile.Log("Begin receiving data on topic " + ATR_Data_Consumer_Globals.sKafkaTopic +
                            ...
                            else if (ATR_Data_Consumer_Globals.iSecurityProtocol == 3)
                            {
                                //Event Hubs consume result. Need to use timeout vs. cancellation token. Cancellation Token does not work with Event Hubs
                                consumeResult = consumer.Consume(ATR_Data_Consumer_Globals.iKafkaConsumerConsumeTimeoutInMillieconds);
