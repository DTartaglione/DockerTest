...
{
    public class MTDC_KafkaConsumer
    {
        /// <summary>
        ///     In this example
        ...
        {
            var sBrokerList = MTDC_Globals.sKafkaBrokerList;
            var sKafkaTopics = MTDC_Globals.sKafkaTopic;

            MTDC_Globals.LogFile.Log("Start Kafka Consumer at: " + sBrokerList + ", listen on topic(s): " + sKafkaTopics);
        ...
        {
            int iCount = 0;
           // MTDC_DataStructures.MTDC_LinkData _LinkData = new MTDC_DataStructures.MTDC_LinkData();
            string sVal = "";
            ...
            var config = new ConsumerConfig
            {
                ...
                GroupId = MTDC_Globals.sKafkaConsumerGroup,
                ...
                try
                {
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            if (!bPrintedOffset)
                            {
                                MTDC_Globals.LogFile.Log("Launched consumer for topic " + consumeResult.Topic + ", beginning at offset: " + consumeResult.Offset + ".");
                                bPrintedOffset = true;
                            }

                            if (consumeResult.IsPartitionEOF)
                            {
                                MTDC_Globals.LogFile.Log("Reached end of topic " + consumeResult.Topic + ", partition " + 
                               ...
                               // consumer.StoreOffset(consumeResult);
                                ...
                                if (lstData.Count > 0)
                                {
                                    MTDC_HandleResponse.HandleResponse(lstData);
                           ...
                           // {
                            ...
                            try
                            {
                                ...
                                {
                                    iCount++;
                                    MTDC_Globals.LogFile.Log("Begin receiving data on topic " + MTDC_Globals.sKafkaTopic + 
