...
{
    public class Waze_Route_Consumer_KafkaConsumer
    {
        /// <summary>
        ///     In this example
        ...
        {
            var sBrokerList = Waze_Route_Consumer_Globals.sKafkaBrokerList;
            var sKafkaTopics = Waze_Route_Consumer_Globals.sKafkaTopic;

            Waze_Route_Consumer_Globals.LogFile.Log("Start Kafka Consumer at: " + sBrokerList + ", listen on topic(s): " + sKafkaTopics);
        ...
        public static void Run_Consume(string brokerList, List<string> topics)
        {
            int iCount = 0;
            string sVal = "";
            List<dynamic> lstKafkaData = new List<dynamic>();
            ...
            var config = new ConsumerConfig
            {
                ...
                GroupId = Waze_Route_Consumer_Globals.sKafkaConsumerGroup,
                ...
                SecurityProtocol = (SecurityProtocol)Waze_Route_Consumer_Globals.iSecurityProtocol, //kafka needs 0, event hubs 3
                SaslMechanism = (SaslMechanism)Waze_Route_Consumer_Globals.iSaslMechanism, //Kafka needs 1, event hubs 1
                SaslUsername = Waze_Route_Consumer_Globals.sSaslUsername,
                SaslPassword = Waze_Route_Consumer_Globals.sSaslPassword
            };
                ...
                try
                {
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            if (Waze_Route_Consumer_Globals.iSecurityProtocol == 0)
                            {
                                ...
                                if (!bPrintedOffset)
                                {
                                    Waze_Route_Consumer_Globals.LogFile.Log("Launched consumer for topic " + consumeResult.Topic + ", beginning at offset: " + consumeResult.Offset + ".");
                                    bPrintedOffset = true;
                                }

                                if (consumeResult.IsPartitionEOF)
                                {
                                    Waze_Route_Consumer_Globals.LogFile.Log("Reached end of topic " + consumeResult.Topic + ", partition " +
                                           consumeResult.Partition + ", offset " + consumeResult.Offset + ".");

                                    if (lstData.Count > 0)
                                    {
                                        Waze_Route_Consumer_HandleResponse.HandleResponse(lstData);
                                ...
                                try
                                {
                                    ...
                                    {
                                        iCount++;
                                        Waze_Route_Consumer_Globals.LogFile.Log("Begin receiving data on topic " + Waze_Route_Consumer_Globals.sKafkaTopic +
                            ...
                            else if (Waze_Route_Consumer_Globals.iSecurityProtocol == 3)
                            {
                                //Event Hubs consume result. Need to use timeout vs. cancellation token. Cancellation Token does not work with Event Hubs
                                consumeResult = consumer.Consume(Waze_Route_Consumer_Globals.iKafkaConsumerConsumeTimeoutInMillieconds);
